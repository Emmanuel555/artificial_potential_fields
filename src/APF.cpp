...
void APF::run(){
    ...
    while(ros::ok()){
     ...
     //   force_publisher.publish(force_msg);
        ...
        resultant_rate(0) = attitude_d(0) - repulsive_force(1); // resultant roll
        resultant_rate(1) = attitude_d(1) + repulsive_force(0); // resultant pitch

        mavros_msgs::AttitudeTarget attitude_msg;
        attitude_msg.header.stamp = ros::Time::now();
        attitude_msg.header.frame_id= "map";
        attitude_msg.body_rate.x = resultant_rate(0);
        attitude_msg.body_rate.y = resultant_rate(1);
        attitude_msg.body_rate.z = attitude_d(2); //ignore z alt rate
        attitude_msg.thrust = attitude_d(3);
        attitude_msg.type_mask = 128;
        attitude_force_publisher.publish(attitude_msg);

        geometry_msgs::Vector3 debug_msg;
        debug_msg.x = attitude_d(0);
        debug_msg.y = attitude_d(1);
        debug_msg.z = attitude_d(2);
